...
public class Json2xml extends IConvertisseur {
	 ...
	 * son contenu en string. Ce string est ensuite transformé en JSONObject, parsé en XML, puis 
	 * retourné en string
	...
	// ne reconnait pas la balise question 
	public String json2string(JSONObject o){
		...
		try {
			...
			if(!contenu.has("question"))
	            return "plop";
			
			reponse += "<question> </question>"; 
			
	        /*String baliseQuestion = "\n<question>";
	        stringName += XML.toString(contenu.get("question"));
	        stringName += "\n</question>";
